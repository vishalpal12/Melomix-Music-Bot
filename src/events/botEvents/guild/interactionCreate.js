...
module.exports.run = async (client, interaction) => {
    if (interaction.type === InteractionType.ApplicationCommand) {
        ...
        if (userBan && userBan.isBanned === true && interaction.user.id !== client.owner) {
            return interaction.reply({
                content: `<:Mark:1147892558610055220> | You are banned from using ${client.user}, click the button support to appeal.`,
                components: [row],
                ephemeral: true,
            });
        }
        ...
        if (!interaction.guild.members.cache.get(client.user.id).permissions.has(command.permissions.bot || [])) {
            warning.setDescription(`<:Mark:1147892558610055220> | I don't have permission \`${command.permissions.bot.join(", ")}\` to execute this command.`);
        ...
        if (!interaction.member.permissions.has(command.permissions.user || [])) {
            warning.setDescription(
                `<:Mark:1147892558610055220> | You don't have permission \`${command.permissions.user.join(", ")}\` to execute this command.`,
            );
        ...
        if (command.settings.player && !player) {
            warning.setDescription(`<:Mark:1147892558610055220> | There isn't player exists for this server.`);
        ...
        if (command.settings.current && !player.currentTrack) {
            warning.setDescription(`<:Mark:1147892558610055220> | There isn't any current playing right now.`);
        ...
        if (command.settings.inVc) {
            if (!channel) {
                warning.setDescription(`<:Mark:1147892558610055220> | You must be in a voice channel to use this command.`);
            ...
            if (
                ...
                warning.setDescription(
                    `<:Mark:1147892558610055220> | I don't have permission \`${command.permissions.channel.join(
                        ", ",
        ...
        if (command.settings.sameVc) {
            if (!channel || interaction.member.voice.channelId !== interaction.guild.members.me.voice.channelId) {
                warning.setDescription(`<:Mark:1147892558610055220> | You must be on the same voice channel as mine to use this command.`);
        ...
        if (command.settings.premium) {
            if (user && !user.isPremium) {
                warning.setDescription(`<:Mark:1147892558610055220> | You're not premium user!`);
        ...
        if (command.settings.owner && interaction.user.id !== client.owner) {
            warning.setDescription(`<:Mark:1147892558610055220> | Only my owner can use this command!\n\nRadioKnight`);
        ...
        try {
            ...
            warning.setDescription(`<:Mark:1147892558610055220> | Something went wrong.`);
